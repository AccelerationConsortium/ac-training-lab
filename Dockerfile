...
# and `pip install ac-training-lab` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ac-training-lab", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ac-training-lab -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ac-training-lab", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ac-training-lab", "YOUR_CONSOLE_SCRIPT"]
