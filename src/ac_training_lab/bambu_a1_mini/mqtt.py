...
# template file path
TEMPLATE_DIR = "/home/linx5o/bambu-printer-control/template"
...
def modify_gcode_template(gcode, nozzle_temp, bed_temp, print_speed, fan_speed):
    ...
    # replace print speed parameter
    # in Bambu G-code, print speed is usually set in the F parameter
    # define base speed (e.g., F3000 corresponds to 100% speed)
    base_speed = 3000  # this value needs to be adjusted according to your actual G-code
    
    # calculate new speed value
    new_speed = int(base_speed * print_speed / 100)
    
    # use regex to replace speed parameter in all movement commands
    gcode = re.sub(r'(G[01].*?F)(\d+)', lambda m: m.group(1) + str(new_speed), gcode)
    
    logger.info(f"replaced print speed in G-code: F{new_speed} (base: F{base_speed}, ratio: {print_speed}%)")
    ...
    # add fan control command after spaghetti detection code
    spaghetti_start = gcode.find("M981 S1 P20000 ;open spaghetti detector")
    if spaghetti_start != -1:
        # find M106 command after spaghetti detection
        fan_off_pos = gcode.find("M106 P2 S0", spaghetti_start)
        if fan_off_pos != -1:
            # add our fan control command after fan off command
            insert_pos = fan_off_pos + len("M106 P2 S0") + 1  # +1 for newline
            gcode = gcode[:insert_pos] + f"M106 P1 S{fan_value}\n" + gcode[insert_pos:]
    
    # close fan after spaghetti detection
    spaghetti_end = gcode.find("M981 S0 P20000 ; close spaghetti detector")
    if spaghetti_end != -1:
        # find position after spaghetti detection
        end_pos = spaghetti_end + len("M981 S0 P20000 ; close spaghetti detector")
        # add fan off command after spaghetti detection
        gcode = gcode[:end_pos] + "\nM106 P1 S0" + gcode[end_pos:]
    
    logger.info(f"replaced fan speed in G-code: M106 S{fan_value} (ratio: {fan_speed}%)")
