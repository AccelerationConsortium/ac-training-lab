...
z_rel_dist_from_bed = 25  # percent, (i.e., don't go too close to 0)


assert z_rel_dist_from_bed > 1  # percent

z_moved_high = printer.gcode(
    f"G1 Z{z_rel_dist_from_bed} F600   ; Move Z to camera position"
)
...
final_xy_position_set = printer.gcode(
    f"G1 X15.25 Y{154.75+y_offset} F3600  ; Final XY position"
)
# ~27 mm offset in Y to get the camera in position
