...
# MQTT topic
# Current scale model/pico id = USS-DBS51-30/e6632c8593690c30
mqtt_topic = f"USS-DBS51-30/{my_id}"
    ...
    while True:
        try:
            ...
            if uart.any(): # Check if there is data available from the scale
                ...
                weight = scale_data.splitlines()[0].strip() if scale_data else "0g"
                current = get_local_time()
                data = OrderedDict([
                    ("Current Weight", weight),
                    ("Date", f"{current[0]:04}-{current[1]:02}-{current[2]:02}"),
                    ("Time", f"{current[3]:02}:{current[4]:02}:{current[5]:02}")
                ])

                message = json.dumps(data)
                print(f"Publishing scale data: {message}")
                await client.publish(mqtt_topic, message, qos=1) # Publish the data to the MQTT topic
