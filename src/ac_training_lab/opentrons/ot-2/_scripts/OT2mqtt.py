...
def mix_color(payload):
    ...
    position = ["B1", "B2", "B3"]  # R,Y,B paint vial on the position of the 3x2 vials rack
   ...
   # client.publish(SENSOR_COMMAND_TOPIC, payloadtosent, qos=2) # send a read commmand to sensor
   
    print("Sending status to HF...")
    payload = f'{{"status": {{"sensor_status":"in_place"}}, "experiment_id": "{experiment_id}", "session_id": "{session_id}"}}'    
...
def move_sensor_back(payload):
    ...
    payload = f'{{"status": {{"sensor_status":"charging"}}, "experiment_id": "{experiment_id}", "session_id": "{session_id}"}}'
    client.publish(OT2_STATUS_TOPIC, payload, qos=2) # send a status message back to HF
...
def handle_command(payload):
    ...
    if {"R", "Y", "B", "well"}.issubset(payload["command"].keys()):
        ...
        print(f"Sensor measure complete")
